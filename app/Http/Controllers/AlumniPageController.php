...
class AlumniPageController extends Controller
{
    ...
    public function __invoke(Request $request): View|JsonResponse
    {
        ...
        $isHonorary = ! $isSearching && in_array($angkatanSlug, [
            HonoraryMember::DIRECTORY_SLUG,
            AlumniBatch::HONORARY_SLUG,
        ], true);

        $selectedBatch = $isSearching
            ? null
            : ($isHonorary
                ? $honoraryBatch
        ...
        $alumniQuery = Alumni::query()
            ...
            ->where('is_public', true)
            ->where('is_active', true)
            ->when(! $isSearching && $selectedBatch, fn ($query) => $query->where('alumni_batch_id', $selectedBatch->id))
            ->when($isSearching, function ($query) use ($search): void {
                $query->where(function ($builder) use ($search): void {
                    $builder
                        ...
                        ->orWhereHas('alumniBatch', function ($batchQuery) use ($search): void {
                            $batchQuery
                                ->where('name', 'like', "%{$search}%")
        ...
        return view('pages.alumni', [
            ...
            'honoraryCount' => $honoraryBatch?->publicMemberCount() ?? 0,
        ]);
    }
}
