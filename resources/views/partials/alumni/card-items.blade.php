@foreach ($alumni as $person)
    <article class="overflow-hidden border border-navy/8 bg-white shadow-sm transition hover:border-gold/35">
        <div class="relative">
            ...
            <span class="absolute top-2 right-2 flex h-7 w-7 items-center justify-center rounded-full bg-navy/85 text-gold backdrop-blur-sm sm:top-3 sm:right-3 sm:h-8 sm:w-8" title="{{ $person->gender === 'female' ? 'Gadis' : 'Bujang' }}">
