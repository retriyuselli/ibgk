@php
    $orgName = $profile->short_name ?? 'IBGK Sumatera Selatan';
    ...
    <div class="site-container relative z-10 pt-8 pb-10 sm:pt-10 lg:pb-14">
        <nav class="mb-8 text-xs text-muted" aria-label="Breadcrumb">
            <ol class="flex flex-wrap items-center gap-2">
                <li><a href="{{ route('home') }}" class="hover:text-banking">Beranda</a></li>
                <li aria-hidden="true">›</li>
                <li><a href="{{ route('partnership') }}" class="hover:text-banking">Kemitraan</a></li>
                <li aria-hidden="true">›</li>
        ...
        <div class="showcase-brand-bar mx-auto flex w-fit max-w-full items-center gap-4 sm:gap-6">
            <div class="flex min-h-12 items-center sm:min-h-14">
                @if (filled($profile?->logo))
                    <img src="{{ asset('storage/'.$profile->logo) }}" alt="{{ $orgName }}" class="h-10 w-auto max-w-[9rem] object-contain sm:h-12 sm:max-w-[11rem]">
                @else
                    <span class="font-display text-base font-semibold text-navy sm:text-lg">{{ $orgName }}</span>
                @endif
            </div>
            <span class="h-10 w-px shrink-0 bg-banking/25 sm:h-12" aria-hidden="true"></span>
        ...
        <div class="mt-10 grid items-center gap-10 lg:grid-cols-2 lg:gap-12 xl:gap-16">
            <div class="max-w-xl">
                <h1 class="font-display text-[1.65rem] leading-[1.15] font-bold tracking-tight text-banking sm:text-3xl lg:text-[2rem] xl:text-[2.15rem]">
                    {{ $partnerShort }}
                    <span class="text-banking">×</span>
                    Bujang Gadis Kampus Sumatera Selatan {{ $year }}
                </h1>
                ...
                @if ($partner->showcase_official_title)
                    <div class="showcase-official-card mt-8 flex gap-4 rounded-lg bg-banking p-5 text-white shadow-lg sm:p-6">
                        <span class="flex h-14 w-14 shrink-0 items-center justify-center rounded-full border-2 border-gold/70 bg-banking-dark text-gold">
            ...
            <div class="showcase-hero-visual relative mx-auto w-full max-w-lg lg:max-w-none">
                <div class="showcase-hero-visual__frame relative overflow-hidden">
                    <div class="showcase-hero-visual__bg absolute inset-0" aria-hidden="true">
                        {!! site_image('images/home/hero-ampera.jpg', 'Jembatan Ampera Palembang', ['class' => 'h-full w-full object-cover opacity-[0.14] grayscale']) !!}
                    </div>

                    @if ($hasHeroPhoto && $partner->heroImageUrl())
                        <img
                            src="{{ $partner->heroImageUrl() }}"
                            alt="Brand ambassador Bujang Gadis Kampus Sumatera Selatan"
                    ...
                    @else
                        <div class="showcase-hero-visual__placeholder relative z-10 flex min-h-[22rem] flex-col items-center justify-center px-8 text-center sm:min-h-[26rem]">
                            <span class="flex h-16 w-16 items-center justify-center rounded-full border border-banking/20 bg-white/80 text-banking shadow-sm">
                            ...
                            <p class="mt-4 text-sm font-semibold text-banking">Bujang & Gadis Kampus Sumsel</p>
                            <p class="mt-1 max-w-xs text-xs leading-relaxed text-muted">Foto brand ambassador dapat diunggah melalui panel admin mitra.</p>
                        </div>
                    @endif
