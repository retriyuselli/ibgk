<section class="showcase-contact-footer showcase-section relative overflow-hidden border-t border-banking/10 bg-[#f4faf9] py-12 sm:py-14">
    ...
    <div class="site-container relative z-10">
        <div class="grid gap-10 lg:grid-cols-2 lg:items-center lg:gap-12">
            <div>
                <h2 class="text-sm font-bold tracking-[0.12em] text-banking uppercase">Hubungi Kami</h2>
                <p class="mt-2 font-display text-xl font-semibold text-navy">{{ $profile->name ?? 'Ikatan Bujang Gadis Kampus Sumatera Selatan' }}</p>

                <ul class="mt-6 space-y-4 text-sm text-muted">
                    @if (filled($profile?->address))
                        <li class="flex gap-3">
                            <span class="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-banking/10 text-banking">
                    ...
                    @if (filled($profile?->phone))
                        <li class="flex gap-3">
                            ...
                            <a href="tel:{{ preg_replace('/\s+/', '', $profile->phone) }}" class="transition hover:text-banking">{{ $profile->phone }}</a>
                        </li>
                    @endif
                    @if (filled($profile?->email))
                        <li class="flex gap-3">
                            ...
                            <a href="mailto:{{ $profile->email }}" class="transition hover:text-banking">{{ $profile->email }}</a>
                        </li>
                    @endif
                    @if (filled($profile?->instagram))
                        <li class="flex gap-3">
                            <span class="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-banking/10 text-banking">
                            ...
                            <a href="{{ $profile->instagram }}" target="_blank" rel="noopener noreferrer" class="transition hover:text-banking">@ibgksumsel</a>
                        </li>
                    @endif
                    @if (filled($profile?->website))
                        <li class="flex gap-3">
                            ...
                            <a href="{{ $profile->website }}" target="_blank" rel="noopener noreferrer" class="transition hover:text-banking">{{ parse_url($profile->website, PHP_URL_HOST) ?: $profile->website }}</a>
                        </li>
                    @endif
                </ul>
            </div>

            <div class="showcase-partner-card relative overflow-hidden rounded-sm border border-banking/12 bg-white px-8 py-10 text-center shadow-sm lg:items-end lg:text-right">
                ...
                @if ($partner->showcase_partner_tagline)
                    <p class="mt-5 max-w-sm font-display text-base leading-relaxed text-banking italic lg:ml-auto">
