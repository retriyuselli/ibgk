@php
    ...
    $initial = strtoupper(substr($subject?->name ?? 'I', 0, 1));
@endphp

@if ($subject?->photo)
    <div @class([$photoClass, 'shrink-0 overflow-hidden rounded-lg border-2 shadow-lg', $borderClass ?? 'border-gold/35'])>
        {!! site_image_or_storage($subject->photo, 'images/home/alumni-placeholder.jpg', $subject->name ?? 'Foto profil', ['class' => 'aspect-[3/4] w-full object-cover']) !!}
    </div>
@else
